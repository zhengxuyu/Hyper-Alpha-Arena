...
def _apply_rate_limiting() -> None:
    ...
    with _global_binance_lock:
        ...
        if time_since_last_call < min_interval:
            ...
            logger.info(f"Rate limiting: sleeping {sleep_time:.2f}s before Binance API call (min interval: {min_interval}s)")
...
def get_binance_balance_and_positions(account: Account) -> Tuple[Optional[Decimal], List[Dict]]:
    ...
    try:
        ...
        if e.code == 401:
            logger.error(f"Binance API authentication failed (401 Unauthorized) for account {account.name}. Please check if the API key and secret key are correct and have proper permissions.")
        elif e.code == 403:
            logger.error(f"Binance API forbidden (403) for account {account.name}. Please check API key permissions.")
        elif e.code == 451:
            logger.error(f"Binance API unavailable (451) for account {account.name}. Service unavailable from restricted location. Please check Binance terms of service.")
        else:
...
def execute_binance_order(
    ...
    try:
        ...
        logger.info(f"Placing Binance order: {side} {quantity} {symbol} @ {price} (pair={pair}, ordertype={ordertype})")
        ...
        if order_id:
            logger.info(f"Binance order placed successfully: orderId={order_id}, pair={pair}, side={side}, quantity={quantity}")
