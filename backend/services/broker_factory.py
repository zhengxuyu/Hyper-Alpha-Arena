...
_DEFAULT_BROKER: Optional[str] = "Kraken"
...
def get_broker(account: Account) -> Optional[BrokerInterface]:
    ...
    if broker_type == "Kraken" or broker_type is None:
        # Use unified api_key and secret_key variable names
        api_key = account.kraken_api_key
        secret_key = account.kraken_private_key
        if not api_key or not secret_key:
            logger.warning(f"Account {account.id} does not have Kraken API keys configured")
            return None
        return KrakenBroker()
