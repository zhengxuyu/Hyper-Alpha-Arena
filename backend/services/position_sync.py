...
Periodically syncs database positions with Kraken actual positions to ensure data consistency.
This addresses the audit report finding about potential inconsistency between DB and Kraken.
"""
import logging
...
from services.broker_adapter import get_positions
...
def sync_account_positions_with_kraken(account: Account, db: Session) -> Dict[str, int]:
    """
    Sync database positions with Kraken actual positions for a single account.
    ...
    if not account.kraken_api_key or not account.kraken_private_key:
        logger.debug(f"Account {account.name} (ID: {account.id}) has no Kraken API keys, skipping sync")
        return {"synced": 0, "removed": 0, "added": 0}
    
    try:
        # Get actual positions from Kraken
        kraken_positions = get_positions(account)
        
        # Create a dict keyed by symbol for easy lookup
        kraken_positions_dict = {}
        ...
        for db_pos in db_positions:
            ...
            if symbol in kraken_positions_dict:
                # Position exists on Kraken - sync it
                kraken_pos = kraken_positions_dict[symbol]
                
                # Only update if there's a significant difference (avoid unnecessary updates)
                qty_diff = abs(float(db_pos.quantity) - kraken_pos["quantity"])
                if qty_diff > POSITION_SYNC_THRESHOLD:  # Use constant
                    db_pos.quantity = kraken_pos["quantity"]
                    db_pos.available_quantity = kraken_pos["available_quantity"]
                    # Update avg_cost if available (Kraken may not always provide this)
                    if kraken_pos.get("avg_cost", 0) > 0:
                        db_pos.avg_cost = kraken_pos["avg_cost"]
                    synced_count += 1
                    logger.debug(
                        f"Synced position {symbol} for account {account.name}: "
                        f"DB={db_pos.quantity} -> Kraken={kraken_pos['quantity']}"
                    )
                else:
                    # Position is in sync
                    pass
                ...
                del kraken_positions_dict[symbol]
            else:
                # Position exists in DB but not on Kraken - remove it
                logger.info(
                    f"Removing position {symbol} from DB (not found on Kraken) for account {account.name}"
                )
                db.delete(db_pos)
                removed_count += 1
        
        # Add new positions that exist on Kraken but not in DB
        for symbol, kraken_pos in kraken_positions_dict.items():
            ...
            position = Position(
                ...
                market="CRYPTO",
                quantity=kraken_pos["quantity"],
                available_quantity=kraken_pos["available_quantity"],
                avg_cost=kraken_pos.get("avg_cost", 0),
            )
            db.add(position)
            added_count += 1
            logger.debug(
                f"Added new position {symbol} from Kraken for account {account.name}: "
                f"quantity={kraken_pos['quantity']}"
            )
...
def sync_all_active_accounts_positions() -> Dict[str, int]:
    """
    Sync positions for all active accounts with Kraken.
    ...
    try:
        ...
        for account in accounts:
            try:
                stats = sync_account_positions_with_kraken(account, db)
                total_stats["synced"] += stats["synced"]
                total_stats["removed"] += stats["removed"]
                total_stats["added"] += stats["added"]
            except Exception as account_err:
                logger.error(
                    f"Failed to sync positions for account {account.name}: {account_err}",
                    exc_info=True
                )
